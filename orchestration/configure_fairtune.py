...
def write_config(repo_dir, img_dir, train_csv, val_csv, test_csv):
    path, cfg = _load_fixed(repo_dir)

    cfg["data"]["papila"]["root_path"] = img_dir
    cfg["data"]["papila"]["img_path"] = img_dir
    cfg["data"]["papila"]["train_csv"] = train_csv
    cfg["data"]["papila"]["val_csv"] = val_csv
    cfg["data"]["papila"]["test_csv"] = test_csv

    for field in REQUIRED_FIELDS:
        val = cfg["data"]["papila"][field]
        assert val, (
            f"config.yaml['data']['papila']['{field}'] is empty/None after writing it. "
    ...
    verify(repo_dir)
    print("Wrote and verified config.yaml:", cfg["data"]["papila"])


def verify(repo_dir):
    """Re-reads config.yaml FRESH OFF DISK (not any in-memory dict) and
    confirms search_mask.py/finetune_with_mask.py will see a valid papila
    entry when they run. Raises with a clear message otherwise.
    ...
    papila_cfg = (cfg or {}).get("data", {}).get("papila")
    if not papila_cfg or not all(papila_cfg.get(k) for k in REQUIRED_FIELDS):
        raise RuntimeError(
            f"{path}'s papila entry is missing/empty: {papila_cfg}. "
...
def main():
    ...
    ap.add_argument("--img-dir", help="Absolute path to FundusImages/ (write mode only)")
    ...
    write_config(args.repo_dir, args.img_dir, args.train_csv, args.val_csv, args.test_csv)
