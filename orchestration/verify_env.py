#!/usr/bin/env python3
"""Fast, FairTune-independent sanity check for the CUDA PyTorch environment.
...
1. CUDA not actually seeing the GPU (silent CPU fallback).
2. A PyTorch DataLoader `num_workers > 0` hang: this builds a tiny synthetic
   ...
   (0 is always safe, try 1-2 first) and rerun this check.
"""
import argparse
import sys
import time
...
def check_device():
    ...
    if not available:
        print("FAIL: no GPU visible to torch. Check `nvidia-smi`, and that the CUDA-enabled torch/torchvision wheels installed cleanly.")
        sys.exit(1)
