...
def create_opt_mask(trial, args, num_blocks):
    ...
    if args.tuning_method in [
        "tune_full_block",
        "tune_attention_blocks_random",
        "tune_layernorm_blocks_random",
        "tune_attention_layernorm",
        "tune_attention_mlp",
        "tune_layernorm_mlp",
        "tune_attention_layernorm_mlp",
    ]:
        mask_length = num_blocks
    elif args.tuning_method == "auto_peft1":
        mask_length = num_blocks * 3
    elif (
        args.tuning_method == "tune_attention_params_random"
        or args.tuning_method == "auto_peft2"
    ):
...
def objective(trial):
    ...
    if args.sens_attribute == "gender":
        if args.use_metric == "acc":
            _row = [
                ...
                args.lr,
                round(val_acc, 3),
                round(val_male_acc, 3),
                round(val_female_acc, 3),
                round(acc_diff, 3),
            ]
        if args.use_metric == "auc":
            _row = [
                ...
                args.lr,
                round(val_auc, 3),
                round(val_male_auc, 3),
                round(val_female_auc, 3),
                round(auc_diff, 3),
            ]

    elif (
        args.sens_attribute == "age"
    ...
    print(
        "!!! Saving the results dataframe at {}".format(
            os.path.join(results_df_savedir, results_df_name)
        )
    )
...
if __name__ == "__main__":
    args = get_args_parser().parse_args()
    args.plots_save_dir = os.path.join(
        os.getcwd(),
        "plots",
        "optuna_plots",
        args.model,
    ...
    if (
        args.objective_metric == "acc_diff"
        or args.objective_metric == "auc_diff"
        or args.objective_metric == "max_loss"
    ):
        direction = "minimize"
    elif (
        args.objective_metric == "min_acc"
        or args.objective_metric == "overall_acc"
        or args.objective_metric == "overall_auc"
        or args.objective_metric == "min_auc"
    ):
        direction = "maximize"
    else:
        raise NotImplementedError
    ...
    if not args.disable_storage:
        ...
        study = optuna.create_study(
            direction=direction, pruner=pruner, storage=storage_name
        )
    else:
    ...
    mask_savedir = os.path.join(
        args.model, args.dataset, "Optuna_Masks", args.sens_attribute
    )
    if not os.path.exists(mask_savedir):
        os.makedirs(mask_savedir)

    print(
        "!!! Saving the best mask at {}".format(
            os.path.join(
                mask_savedir,
                args.tuning_method
                + "_best_mask_"
    ...
    best_params_df.to_csv(
        os.path.join(stats_df_savedir, best_params_df_name), index=False
    )
